  ...
  return (
    <div className="lg:col-span-7 space-y-4">
      ...
      <div className="relative">
        <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-zinc-555 text-zinc-550 text-zinc-500" />
        <input 
          type="text" 
      ...
      {filteredDivisions.map((division) => {
        ...
        return (
          <div 
            key={division.id} 
            className="p-5 bg-zinc-900/30 backdrop-blur-md border border-zinc-850 hover:border-zinc-800 rounded-2xl transition-all group flex flex-col gap-4"
          >
            <div className="flex items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-2 shrink-0">
                <Link
                  ...
                  className="p-2 bg-zinc-950 hover:bg-zinc-800 border border-zinc-800 hover:border-zinc-700 rounded-lg text-zinc-500 hover:text-white transition-all shadow-sm"
                >
                ...
                {hasTeams ? (
                  <button 
                    ...
                    className="p-2 bg-zinc-950/40 border border-zinc-900/60 rounded-lg text-zinc-800 cursor-not-allowed flex items-center justify-center"
                    disabled
                  >
                  ...
                  <form action={deleteDivisionAction}>
                    ...
                    <button 
                      ...
                      className="p-2 bg-zinc-950 hover:bg-rose-950/40 border border-zinc-800 hover:border-rose-900/40 rounded-lg text-zinc-500 hover:text-rose-400 transition-all shadow-sm cursor-pointer"
                    >
            ...
            <div className="pt-3 border-t border-zinc-850/60 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div className="flex items-center gap-1.5 text-[10px] font-black uppercase text-zinc-550">
                <Users className="w-3.5 h-3.5 text-zinc-600 shrink-0" />
                {hasTeams ? (
                  <span className="text-zinc-400">{teamCount} {teamCount === 1 ? 'Team' : 'Teams'} Enrolled</span>
                ) : (
                  <span className="text-zinc-600 italic">Empty Bracket (0 Teams)</span>
                )}
              </div>
                ...
                <div className="flex flex-wrap gap-1">
                  {division.teams.map((t) => (
                    <div 
                      key={t.id}
                      title={t.name}
                      className="flex items-center gap-1 bg-zinc-950 border border-zinc-850 px-2 py-0.5 rounded-md max-w-[120px] text-zinc-400 font-bold"
                    >
                      {t.logoUrl ? (
