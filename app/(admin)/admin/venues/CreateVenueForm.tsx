...
export default function CreateVenueForm({ createVenueAction }: CreateVenueFormProps) {
  ...
  return (
    <div className="bg-slate-900/40 backdrop-blur-md rounded-[2rem] border border-slate-800 p-6 md:p-8 relative overflow-hidden shadow-2xl">
      ...
      <p className="text-[10px] text-slate-500 font-bold uppercase mb-6">Initialize a new match deployment hub</p>

      <form action={formAction} className="space-y-5">
        <div className="space-y-2">
          <label className="text-[10px] font-black uppercase tracking-wider text-slate-400 ml-1">Club Venue Name</label>
          <input 
            type="text" 
            name="name" 
            required 
            placeholder="e.g., Rack & Cue Arena" 
            className={`w-full p-3.5 bg-slate-950 border ${state?.error ? 'border-rose-500/50 focus:border-rose-500' : 'border-slate-800 focus:border-indigo-500'} rounded-xl outline-none font-bold text-slate-100 text-xs transition-all placeholder:text-slate-500`}
          />
        ...
        <div className="space-y-2">
          <label className="text-[10px] font-black uppercase tracking-wider text-slate-400 ml-1">Street Address Locator</label>
          <input 
            type="text" 
            name="address" 
            placeholder="e.g., 42 Breakaway Blvd, Tier 1" 
            className="w-full p-3.5 bg-slate-950 border border-slate-800 focus:border-indigo-500 rounded-xl outline-none font-bold text-slate-100 text-xs transition-all placeholder:text-slate-500"
          />
        </div>

        <div className="space-y-2">
          <label className="text-[10px] font-black uppercase tracking-wider text-slate-400 ml-1">Initial Node State</label>
        ...
        <button 
          type="submit" 
          disabled={isPending}
          className="w-full bg-indigo-600 hover:bg-indigo-500 disabled:bg-slate-800 disabled:text-slate-500 text-white p-4 rounded-xl font-black uppercase tracking-widest text-[10px] transition-all shadow-md shadow-indigo-900/10 active:scale-[0.98] mt-2 cursor-pointer"
        >
