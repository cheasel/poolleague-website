        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filteredVenues.map((venue) => {
            ...
            return (
              <div 
                key={venue.id} 
                className={`bg-zinc-900/30 backdrop-blur-md border hover:border-zinc-800/80 rounded-2xl p-5 flex flex-col justify-between group transition-all relative overflow-hidden ${
                  venue.isActive ? 'border-zinc-850 hover:border-indigo-500/20' : 'border-zinc-900 opacity-75'
                }`}
              >
                <div className="space-y-4">
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <div className="relative">
                      <div className={`w-10 h-10 rounded-xl bg-zinc-950 border flex items-center justify-center transition-colors shrink-0 shadow-inner ${
                        venue.isActive ? 'border-zinc-800 text-zinc-500 group-hover:text-indigo-400 group-hover:border-indigo-500/20' : 'border-zinc-900 text-zinc-700'
                      }`}>
                        <Store className="w-4 h-4" />
                      </div>
                      
                      {/* Mini capacity dot */}
                      <span className={`absolute -top-1 -right-1 w-3.5 h-3.5 rounded-full text-[8px] font-black flex items-center justify-center border border-zinc-950 shadow-md ${
                        isFull 
                  ...
                  <div>
                    ...
                    {venue.address ? (
                      <a 
                        ...
                        className="flex items-center gap-1.5 text-[11px] text-zinc-500 hover:text-indigo-400 font-medium mt-1.5 truncate transition-colors group/link"
                      >
                        <MapPin className="w-3 h-3 text-zinc-650 shrink-0 group-hover/link:text-indigo-400" />
                        <span className="underline decoration-dotted decoration-zinc-700 truncate">{venue.address}</span>
                        <ExternalLink className="w-2.5 h-2.5 text-zinc-700 opacity-0 group-hover/link:opacity-100 transition-all shrink-0" />
                      </a>
                    ) : (
                      <p className="flex items-center gap-1.5 text-[11px] text-zinc-650 italic font-medium mt-1.5 truncate">
                        <MapPin className="w-3 h-3 text-zinc-750 shrink-0" />
                        No Address Provided
                      </p>
                    )}
                  </div>
                  ...
                  {teamCount > 0 && (
                    <div className="space-y-1.5 pt-1.5">
                      <div className="text-[8px] font-black uppercase tracking-wider text-zinc-600 ml-0.5">Franchise Tenants:</div>
                      <div className="flex flex-wrap gap-1.5">
                        {venue.teams.map((t) => (
                          <div 
                            key={t.id} 
                            className="flex items-center gap-1 bg-zinc-950 border border-zinc-850 px-2 py-0.5 rounded-md max-w-full text-zinc-400 font-bold"
                          >
                            {t.logoUrl ? (
                ...
                <button
                  ...
                  className={`w-full mt-5 pt-3 border-t border-zinc-850/60 flex items-center justify-between text-[10px] font-black uppercase tracking-wider text-zinc-400 cursor-pointer hover:bg-zinc-950/20 transition-all rounded px-1.5 py-1 -mx-1.5 ${
                    isUpdating ? 'animate-pulse' : ''
                  }`}
                >
                  <span className="flex items-center gap-1.5 text-zinc-500">
                    <ToggleLeft className={`w-3.5 h-3.5 transition-colors ${venue.isActive ? 'text-indigo-400' : 'text-zinc-650'}`} /> Operational State
                  </span>
