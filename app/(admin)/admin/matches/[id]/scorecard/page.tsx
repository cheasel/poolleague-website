...
export default async function MatchScorecardPage({ params }: { params: { id: string } }) {
  ...
  // Guard against null parameters for away team lookup to satisfy strict typescript overloads
  ...
  // 2. Guard both team IDs to guarantee they are numbers before executing player lookups
  ...
  // 3. Fetch existing frames logged for this specific fixture match sheet
  ...
  // NEW: DELETE FRAME ACTION PIPELINE
  async function deleteFrameRow(formData: FormData) {
    "use server";
    const gameId = Number(formData.get("gameId"));
    
    // 1. Delete target frame row
    await db.delete(matchGames).where(eq(matchGames.id, gameId));

    // 2. Refetch and normalize order positions for remaining games sequentially
    ...
    // 3. Sync matching team scores metrics
    await syncMatchTotals(matchId);
  ...
  return (
    <div className="space-y-8 pb-16 max-w-6xl mx-auto">
      <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <Link href="/admin" className="inline-flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest hover:text-indigo-600 transition-all">
          <ArrowLeft className="w-4 h-4" /> Exit to Control Command Center
        </Link>
          ...
          {existingFrames.map((frame) => {
            ...
            return (
              ...
              >
                ...
                {/* Score Submission Form Core */}
                <form action={saveFrameScores} className="grid grid-cols-1 lg:grid-cols-10 gap-4 items-center lg:col-span-10">
                  ...
                  <div className="hidden">
                    <button type="submit" id={`submit-btn-${frame.id}`} />
                  </div>
                </form>

                {/* Control Tool Box Actions Block */}
                <div className="lg:col-span-1 flex lg:flex-col items-center justify-end gap-2 border-t lg:border-t-0 pt-4 lg:pt-0 border-slate-100 w-full lg:w-auto">
                  {/* Trigger Main Form Submit Button */}
                  <button 
                    onClick={() => document.getElementById(`submit-btn-${frame.id}`)?.click()}
                    className="p-2.5 text-[10px] font-black uppercase tracking-widest bg-indigo-50 text-indigo-600 hover:bg-indigo-600 hover:text-white rounded-xl transition-all grow lg:grow-0 text-center"
                  >
                    Save
                  </button>

                  {/* DESTRUCTIVE ACTION FORM ENTRY */}
                  <form action={deleteFrameRow} className="grow lg:grow-0">
                    <input type="hidden" name="gameId" value={frame.id} />
                    <button className="w-full p-2.5 bg-rose-50 text-rose-500 hover:bg-rose-600 hover:text-white rounded-xl transition-all flex items-center justify-center">
