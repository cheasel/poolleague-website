...
export default function AdminLayout({ children }: AdminLayoutProps) {
  ...
  const renderNavList = (items: typeof operationalItems, closeMobile = false) => {
    return items.map((item) => {
      ...
      // 🎯 FIXED: Check if current path exactly matches OR is a sub-route of this link
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex text-slate-800 antialiased">
      ...
      <aside className="hidden lg:flex flex-col w-72 bg-slate-950 p-6 border-r border-slate-900 shrink-0 sticky top-0 h-screen justify-between z-20">
        <div className="space-y-7 overflow-y-auto scrollbar-none">
          ...
          <nav className="space-y-1">
            <span className="text-[9px] font-black uppercase tracking-[0.2em] text-slate-600 block mb-2.5 px-3">Infrastructure</span>
            {renderNavList(structuralItems)}
          </nav>

          <nav className="space-y-1">
            <span className="text-[9px] font-black uppercase tracking-[0.2em] text-slate-600 block mb-2.5 px-3">Core Operations</span>
        ...
        <div className="border-t border-slate-900 pt-4 flex items-center justify-between mt-4">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center text-xs font-black text-white uppercase shadow-inner">OP</div>
            <div>
          ...
          <button title="Sign Out Session" className="w-8 h-8 rounded-lg hover:bg-red-950/40 hover:text-red-400 flex items-center justify-center transition-colors text-slate-600">
            <LogOut className="w-4 h-4" />
          </button>
        </div>
      </aside>

      {/* 2. MOBILE FLOATING HEADER */}
      <div className="lg:hidden fixed top-0 left-0 right-0 h-16 bg-slate-950 border-b border-slate-900 px-4 flex items-center justify-between text-white z-30 shadow-md">
        ...
        <button onClick={() => setIsMobileOpen(!isMobileOpen)} className="w-10 h-10 rounded-xl bg-slate-900 border border-slate-800 flex items-center justify-center text-slate-400 hover:text-white transition-all outline-none">
