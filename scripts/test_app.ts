...
// Helper to query an endpoint and measure response time with timeout safeguard
async function testEndpoint(url: string): Promise<{ status: number; timeMs: number; sizeBytes: number }> {
  ...
  return new Promise((resolve) => {
    ...
    // Set a timeout of 10 seconds to prevent tests from hanging indefinitely
    req.setTimeout(10000, () => {
      req.destroy();
      resolve({ status: 504, timeMs: Date.now() - start, sizeBytes: 0 });
    });
  });
}
